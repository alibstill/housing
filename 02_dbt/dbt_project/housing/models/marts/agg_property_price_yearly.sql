...
WITH aggregated_yearly_property_price_data AS (
    ...
    FROM {{ ref('fact_property_sales')}} ps
    INNER JOIN {{ ref('dim_locations') }} ll ON ps.location_id = ll.location_id
    INNER JOIN{{ ref('dim_dates')}} dd on ps.date_of_transfer = dd.day_date
    INNER JOIN {{ ref('dim_property_types')}} pt on ps.property_type_id = pt.property_type_id
    INNER JOIN {{ ref('dim_tenures')}} t on ps.tenure_id = t.tenure_id
